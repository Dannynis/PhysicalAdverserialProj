...
def optimize_patch():
    ...
    # mapper = lambda x: jitter_total_photo(augmentor(x))
    ...
    for epoch in range(30):
        for frames, H_t in tqdm.tqdm(train_loader):
            ...
            if i < 1:
                mapper = lambda x: jitter_total_photo(x)

            else:
                mapper = lambda x: jitter_total_photo(torch.stack([augmentor(xx) for xx in x]))
