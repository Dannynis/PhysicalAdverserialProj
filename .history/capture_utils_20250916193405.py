...
def photometric_calibration():
    ...
    resizer = torchvision.transforms.Resize((height))
    ...
    P = np.stack([patterns['red_image'], patterns['green_image'], patterns['blue_image'], patterns['red_image_2'], patterns['green_image_2'], patterns['blue_image_2'], patterns['red_image_3'], patterns['green_image_3'], patterns['blue_image_3']], axis=0)
    C = np.stack([captured['red_image'], captured['green_image'], captured['blue_image'], captured['red_image_2'], captured['green_image_2'], captured['blue_image_2'], captured['red_image_3'], captured['green_image_3'], captured['blue_image_3']], axis=0)

    resized_P = np.stack([cv2.resize(img, (width, height)) for img in P])
    ...
    with open('photometric_calibration.pkl', 'wb') as f:
        pickle.dump({
            ...
            'width': width,
            'height': height,
            'orig_proj_corners': orig_proj_corners,
            'orig_proj_striped_corners': orig_proj_striped_corners,
            'orig_rect_corners': orig_rect_corners,
        }, f)